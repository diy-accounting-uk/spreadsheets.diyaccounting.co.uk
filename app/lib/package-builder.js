...
// The third layer's terms and the line every package doc carries.
export const SPDX_LICENSE_ID = "LicenseRef-PolyForm-Internal-Use-1.0.0";
export const COPYRIGHT_LINE = "Copyright (C) 2006-2026 DIY Accounting Limited";
export const SOURCE_URL = "https://github.com/diy-accounting-uk/spreadsheets.diyaccounting.co.uk";
export const DOWNLOAD_URL = "https://spreadsheets.diyaccounting.co.uk/download.html";

/**
 * The display name for a licence, taken from the first non-blank line of its
 * text. build-packages.js reads this from whatever the root LICENSE file
 * holds at build time, so the name always matches the text copied beside it.
 */
export function licenceNameFromText(licenceText) {
  const firstLine = licenceText.split(/\r?\n/).find((line) => line.trim().length > 0);
  return firstLine ? firstLine.trim() : "";
}

/**
 * README.txt content for a package zip: what the package is, its licence,
 * and where to find the source and the latest download.
 */
export function buildReadmeText(productName, date, licenceName) {
  const label = dateToLabel(date);
  return [
    `${productName} — for the year to ${label}`,
    "",
    `Licence: ${licenceName}`,
    "",
    "Use this for your own accounts, or your clients' accounts if you are an accountant.",
    "Do not redistribute this package. Do not host it, or a changed copy of it, under another name.",
    "",
    COPYRIGHT_LINE,
    "",
    `Source: ${SOURCE_URL}`,
