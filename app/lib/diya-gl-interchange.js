...
//
// diya-gl-interchange.js — what a byte array handed to the CLI, the MCP
...
const JSON_FORMAT = "diya-gl";
const JSON_VERSION = 1;

// A file written before the format's name changed from "diya-gl-books" to
// "diya-gl" carries the old name in its "format" field. The version number
// alone did not change, so a legacy file still reads once its format name is
// recognised here -- the writer only ever emits JSON_FORMAT.
const LEGACY_JSON_FORMATS = ["diya-gl-books"];
...
/** A JSON file that is not this module's own diya-gl format. */
...
// { "format": "diya-gl", "version": 1, "product": "bst", "book": {...}, "lines": [...] }
// A file saved before the format's rename still reads: its "format" is one
// of LEGACY_JSON_FORMATS rather than JSON_FORMAT.
function parseJsonDocument(text) {
  ...
  if (document.format !== JSON_FORMAT && !LEGACY_JSON_FORMATS.includes(document.format)) {
    const expected = [JSON_FORMAT, ...LEGACY_JSON_FORMATS].map((f) => JSON.stringify(f)).join(" or ");
    throw new InvalidDiyaGlJsonError(`expected "format": ${expected}, found ${JSON.stringify(document.format)}`);
  }
