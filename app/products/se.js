...
export function checkCompliance(results, expected, taxData, calculateExpectedTax) {
  ...
  if (paymentSchedule && payrollYearOpens) {
    const payrollYearOpensSerial = toExcelSerial(payrollYearOpens.getUTCFullYear(), payrollYearOpens.getUTCMonth() + 1, payrollYearOpens.getUTCDate());
    PAYE_SCHEDULE_MONTH_TABS.forEach((tab, taxMonth) => {
      ...
      const dated = excelSerialToUtcDate(monthEndSerial);
      ...
      if (dated.getUTCDate() !== lastDay.getUTCDate()) {
        checks.push({
          ...
          actual: monthEndSerial,
          expected: toExcelSerial(lastDay.getUTCFullYear(), lastDay.getUTCMonth() + 1, lastDay.getUTCDate()),
          pass: false,
          severity: "warning",
          diff: "",
        });
