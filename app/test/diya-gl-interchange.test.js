...
//
// diya-gl-interchange.test.js — detectBookSource sniffs six byte kinds by
...
// baseline (taxiOvertypeTemplate() in diya-gl-interchange.js) is always built
...
describe("readBookSource: every kind reaches the same D", () => {
  ...
  it("reads a JSON file stamped with the format's old name to the same D", async () => {
    const legacyDocument = { ...JSON.parse(referenceJson), format: "diya-gl-books" };
    const bytes = new TextEncoder().encode(JSON.stringify(legacyDocument));
    const source = await readBookSource(bytes, "book-old-stamp.json");
    expect(source.kind).toBe("json");
    expectSameBook(source);
  });
...
describe("a multi-file package, sniffed by the files and sheets it carries", () => {
  ...
  it("reads an SE package zip to the same D as export.js --package se --source-dir", async () => {
    const outputDir = mkdtempSync(join(tmpdir(), "diya-gl-interchange-se-"));
...
// diya-gl-interchange.js's readWorkbookSource.
...
describe("the Taxi anchor table", () => {
  ...
  // diya-gl-interchange.js), not a static template file the way BST's and SE's
...
describe("writeBookJson: canonical key order, round trips to the same D", () => {
  it("carries the diya-gl envelope", () => {
    const document = JSON.parse(referenceJson);
