...
import { loadTaxDataForBook, saveWorkbookFiles } from "../lib/product-workbook.js";
...
// The SE package these tests read against: written fresh from the current
// templates through the product writer, rather than the committed
// examples/se-latest fixture. That fixture is regenerated by CI from
// whichever template shipped when it last ran, so a template formula edit
// between refreshes leaves it carrying a stale literal where the current
// template now computes -- which the overtype-sidecar cases below would
// otherwise report as an extra typed-over cell nobody introduced.
const SE_ADVANCED_BOOK_DIR = resolve(ROOT, "examples", "precision-code-ltd", "advanced");

async function builtSePackageDir() {
  const { book, lines } = loadDiyaGlData(SE_ADVANCED_BOOK_DIR);
  const { files } = await saveWorkbookFiles(book, lines);
  const dir = mkdtempSync(join(tmpdir(), "se-package-current-"));
  for (const file of files) writeFileSync(resolve(dir, file.name), file.bytes);
  return dir;
}

const SE_PACKAGE_DIR = await builtSePackageDir();
afterAll(() => rmSync(SE_PACKAGE_DIR, { recursive: true, force: true }));
