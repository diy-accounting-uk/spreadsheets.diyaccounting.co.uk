...
until `/board` shows no machine-only row in state `ready-to-start` or `ready-to-resume`. This
skill names the cycle, fixes its order, and carries the rules the do-next and watch skills already
hold. The parts are the existing skills; this one is the conductor.
...
3. **Inboxes.** `~/.claude/inboxes/spreadsheets.md`, `~/.claude/inboxes/diyaccounting.md` and the
...
- **One batch branch per wave** (`claude/b<n>-board`, its own worktree under
  `../.worktrees/spreadsheets/b<n>`), every agent worktree branched from it, `NEXT.md` never on
  it.
...
- **Fill until file contention stops you**, then stop; there is no target count. Every batch
  must finish inside what is left of the SSO window, including its deploy (about 8 minutes here;
  this repo deploys in place, so there is no per-branch stack to tear down) and one redeploy for a
  fix.
...
- **Every brief carries** the worktree path and branch, absolute paths for every file tool,
  ...
  must grant, `--repo` on `gh` with no checkout, grep the siblings for the same defect.
...
`/watch` arms `scripts/watch-ci.sh` as a Monitor for 75 seconds a cycle; re-arm on every expiry
notice while anything in scope is in flight. A red is diagnosed from the run's own jobs and logs
...
commit; never push while a deploy of that branch is in flight — this repo has two deploy
workflows, `deploy.yml` and `deploy-holding.yml`, so check each.
...
- With nothing startable and the next unblock past the SSO window or on another day, stop the
  ...
  the next scheduled event: on 2026-09-21 the submit loop stopped at 14:12 naming a snapshot run
  due the next day, while the `PARKED.md` triage that restarted it at 19:08 sat unmentioned in an
  earlier reply. The loop still stops; the operator restarts it with `/iterate`.
...
an AWS write outside a workflow, and a Stripe live key are the operator's. Print each as one
fenced block with the `!` prefix at the moment it arises, collect them again in the next board's
Part 4, and carry on; nothing waits on them. Never ask another session to run one.

## Report

Each cycle ends with one line per PR merged (number, rows, merge commit), the live prod stack's
serving state, and what the next cycle starts with or why the loop stopped.
