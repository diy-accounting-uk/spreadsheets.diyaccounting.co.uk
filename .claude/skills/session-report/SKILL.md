...
hand-written change. `packages/` and `examples/*-latest` are generated package output, and
`reports/` holds reconciliation output, not hand-written prose or code; count all three apart:
...
for id in $(gh run list --limit 200 --created ">=<session start ISO>" --json databaseId --jq '.[].databaseId'); do
  gh api "repos/diy-accounting-uk/spreadsheets.diyaccounting.co.uk/actions/runs/$id/jobs?per_page=100" \
...
commit), supersessions, reruns after contention, or docs-only triggers. A public repository bills
...
(static, per-branch, ephemeral), and any metered call the workflows made (the `reconciliation-judge`
LLM judge, a Bedrock call). Cost from the account's structure; name the source.
...
the session's behaviour, a new requirement. The pastes and corrections carry the lessons; the new
requirements are listed for the operator to place.
...
3. **What worked**: one table, a row per efficiency (elapsed time, LLM cost, AWS cost, GitHub
   ...
   file boundary, a worktree-per-agent.
4. **Room for improvement**: one table, a row per loss, the measured size of the loss, the cause,
   ...
   being reported.
   Losses this repository produces by shape: a ci deploy queued behind another because
   `deploy.yml`'s concurrency group does not cancel in progress; a scheduled generate or publish
   workflow colliding with a deploy's reconciliation checks; an agent that ends its turn on a long
   foreground generate or test run instead of backgrounding it.
...
Prettier the report alone (`npx prettier --write REPORT_SESSION_*.md`; never reformat `NEXT.md`),
run `npx vitest --run app/test/licence-headers.test.js` (the report needs the header comment),
commit the report alone as a docs-only commit to `main`, and push. Then say, in the
