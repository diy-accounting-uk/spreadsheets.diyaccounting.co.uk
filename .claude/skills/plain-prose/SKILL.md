...
`web/spreadsheets.diyaccounting.co.uk/public/`, the `CONTEXT_*.md` product docs, the `PLAN_*.md`
design docs, reconciliation reports under `reports/`, other skill docs, code comments, and the
assistant's own chat responses.
...
- **Active voice, not passive.** "The pipeline recalculates it," not "it is recalculated by the
  pipeline." Passive hides who does what and adds words.
...
- **The "not X, it's Y" / "not X but Y" / "not only X but also Y" negation-contrast.** State what
  the thing is, not what it isn't. "One package reconciled, four did not," not "it is not a clean
  run."
...
- **Anthropomorphizing tools and pipelines.** A recalculation does not "want," a workbook does not
...
Reader-facing surfaces (`README.md`, the site pages under
`web/spreadsheets.diyaccounting.co.uk/public/`) sell the product. They are not the place to prove
every claim. Someone landing on a page wants to know what the product does and how to get it,
before any methodology.
...
- Link out for the proof: point to the relevant `CONTEXT_*.md` doc or a `reports/` reconciliation
  report instead of reproducing its tables.

**Keep the full detail in `CONTEXT_*.md`** (sheet maps, formula references, CI pipeline
descriptions per product) and in `reports/` (reconciliation pass/fail tables, cell appendices).
The `PLAN_*.md` docs hold the design-level detail behind a change in progress. A reader who wants
to verify follows the link and finds everything; a reader who just wants to know what the product
does is not made to wade through the proof.
...
- **No delta-framing.** Describe the work on its own terms. Don't frame a change as a rebuttal to
  a prior approach or a competitor. If a reconciliation run refuted an approach, report the
...
4. **On a reader-facing surface, check the order.** Benefit first, then how to get it, then a
   short claim with a link to the relevant `CONTEXT_*.md` or `reports/` doc. If methodology
   arrives before benefit, move it.
...
sentence, and link to the proof instead of reproducing it; the full detail lives in the
`CONTEXT_*.md` docs, the `reports/` reconciliation reports, and the design detail in `PLAN_*.md`
docs. Base rules are the Plain English Campaign's (plainenglish.co.uk). Applies to docs, code
