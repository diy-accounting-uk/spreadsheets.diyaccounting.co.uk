...
gh api graphql -f query='{repository(owner:"diy-accounting-uk",name:"spreadsheets.diyaccounting.co.uk"){pullRequest(number:N){reviewThreads(first:100){nodes{isResolved isOutdated comments(first:1){nodes{author{login} body path}}}}}}}'
```
...
```bash
gh api repos/diy-accounting-uk/spreadsheets.diyaccounting.co.uk/pulls/<n> --jq '"merged=\(.merged) head=\(.head.sha[0:8]) mergeCommit=\(.merge_commit_sha[0:8])"'
