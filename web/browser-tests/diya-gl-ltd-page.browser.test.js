...
// web/browser-tests/diya-gl-ltd-page.browser.test.js
//
// LT-T7's own proof: the Ltd page (diya-gl/ltd.html) boots on a real,
...
// (diya-gl-se-equivalence.browser.test.js): a File built from the zip bytes,
...
test.describe("DIYA-GL page — Ltd page boots on a real package (LT-T7)", () => {
  ...
  test("every data-r-key the page renders exists in report.js --data output for the same package", async ({ page }) => {
    ...
    // shared render-coverage sweep (diya-gl-render-coverage.browser.test.js)
