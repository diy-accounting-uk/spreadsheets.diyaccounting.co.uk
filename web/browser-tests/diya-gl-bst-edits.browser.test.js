...
// web/browser-tests/diya-gl-bst-edits.browser.test.js
...
// mechanism diya-gl-formats.browser.test.js uses for A1/A2, kept local here so
...
test.describe("DIYA-GL page — in-place edits", () => {
  test("an added purchase of X lowers profit by X and leaves turnover alone", async ({ page }) => {
    ...
    await page.screenshot({ path: path.join(screenshotsDir, "diya-gl-bst-entries-grid-edit.png"), fullPage: false });
  });
...
test.describe("DIYA-GL page — the rung: helpers fix a deliberately broken book", () => {
  ...
  test("an entry dated outside the period is caught and moved back into it", async ({ page }) => {
    ...
    await page.screenshot({ path: path.join(screenshotsDir, "diya-gl-bst-helper-preview.png"), fullPage: false });
...
test.describe("DIYA-GL page — drift after an edit", () => {
  test("an uploaded workbook's drift annotations relabel as recalculated once the book is edited", async ({ page }) => {
    ...
    await page.screenshot({ path: path.join(screenshotsDir, "diya-gl-bst-edit-recalculated.png"), fullPage: false });
  });
});
