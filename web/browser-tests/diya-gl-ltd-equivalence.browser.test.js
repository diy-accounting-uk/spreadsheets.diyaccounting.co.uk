...
// web/browser-tests/diya-gl-ltd-equivalence.browser.test.js
//
// The A-series for the Company page (diya-gl/ltd.html): does the browser show
...
// boxes are the Ltd forms spec's subject (diya-gl-ltd-forms.browser.test.js
  ...
  for (const example of SCENARIOS_LTD) {
    test(`${example.scenario}: every rendered figure matches S2`, async ({ page }) => {
      ...
      for (const [key, { text, className, wholePounds }] of rendered) {
        ...
        // its own sweep (diya-gl-render-coverage.browser.test.js).
        if (!s2Entry) continue;
...
test.describe("DIYA-GL Company books page — a true package upload (A7)", () => {
  ...
  test("a corrupted leaf cell shows exactly that cell's drift", async ({ page }) => {
    ...
    // case (diya-gl-ltd-forms.browser.test.js, the computation's margin).
