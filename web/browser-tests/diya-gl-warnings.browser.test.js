...
// web/browser-tests/diya-gl-warnings.browser.test.js
//
...
test.describe("DIYA-GL page — E2: deliberate warnings and failures", () => {
  ...
  test("an entry posted outside the chart flips book-accounts-in-chart alone", async ({ page }) => {
    ...
    await page.evaluate(async () => {
      ...
      await window.DiyaGlPage.setLines(snapshot.lines.concat([imported]), "test: import an out-of-chart line");
    });
  ...
  test("an edited mileage quantity moves the claim but leaves both mileage checks passing", async ({ page }) => {
    ...
    const before = await page.evaluate(() => window.DIYA_GL_SNAPSHOT.report.values.find((v) => v.key === "cell/PurchasesMar!C1").value);
    ...
    await page.evaluate(async () => {
      ...
      await window.DiyaGlPage.setLines(lines, "test: edit the mileage claim's quantity");
    });
    await page.waitForFunction(() => window.DIYA_GL_SNAPSHOT.edited === true);

    const after = await page.evaluate(() => window.DIYA_GL_SNAPSHOT.report.values.find((v) => v.key === "cell/PurchasesMar!C1").value);
