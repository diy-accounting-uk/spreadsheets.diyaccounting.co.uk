...
// web/browser-tests/diya-gl-empty-state.browser.test.js
//
...
// A real HTTP origin, exactly like diya-gl-bst.browser.test.js: the engine's
...
test.describe("DIYA-GL page — new-book form", () => {
  test("the form creates and renders an empty, honest book", async ({ page }) => {
    ...
    await page.screenshot({ path: path.join(screenshotsDir, "diya-gl-new-book-form.png"), fullPage: false });
    ...
    const snapshot = await page.evaluate(() => {
      const s = window.DIYA_GL_SNAPSHOT;
      return {
...
test.describe("DIYA-GL page — IndexedDB autosave", () => {
  test("a loaded book survives a reload via the continue offer; discard clears it", async ({ page }) => {
    ...
    await page.screenshot({ path: path.join(screenshotsDir, "diya-gl-continue-offer.png"), fullPage: false });
