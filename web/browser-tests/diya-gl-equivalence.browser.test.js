...
// web/browser-tests/diya-gl-equivalence.browser.test.js
//
...
// Mirrors diya-gl-render-coverage.browser.test.js's own sweep, but keeps each
...
// The same mechanics as diya-gl-bst.browser.test.js's own corruptedCachedValue,
  ...
  for (const example of SCENARIOS) {
    test(`${example.scenario}: every rendered figure matches S2`, async ({ page }) => {
      ...
      for (const [key, { text, className }] of rendered) {
        const s2Entry = s2Map.get(key);
        if (!s2Entry) continue; // A5 (diya-gl-render-coverage) proves every rendered key is one S2 carries.
        compared++;
...
// Mirrors diya-gl-se-equivalence.browser.test.js's own A9: every box in
