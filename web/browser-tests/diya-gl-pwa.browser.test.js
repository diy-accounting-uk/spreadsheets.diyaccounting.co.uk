...
// web/browser-tests/diya-gl-pwa.browser.test.js
//
// The books pages install as a PWA (diya-gl/manifest.webmanifest,
// diya-gl/sw.js) and keep working with the network cut off. Proves both: the
...
test.describe("books PWA offline", () => {
  test("the BST page reloads offline and an example book still renders", async ({ page, context }) => {
    ...
    expect(cacheNames.some((name) => name.startsWith("diya-gl-"))).toBe(true);
