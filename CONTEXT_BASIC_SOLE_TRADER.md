...
Two test scenarios exist for BST:

### bst-scenario-basic.toml

**"Basic web designer"** -- a sole trader web designer with steady monthly income.

- **Sales**: 36,000 total (3,000/month across 12 months, no other income)
- **Purchases**: premises rent (12x400=4,800), phone/internet (4x120=480), insurance (350), laptop (1,200), accountant (500)
- **Expense codes used**: p (premises), g (gen admin), l (legal & prof), f (fixed assets)
- **Stock**: none
- **Expected P&L**: gross profit 36,000 (no CoS), net profit 29,870
- **Expected tax (2025-26)**: income tax 3,460, NI Class 4 1,038, total 4,498

### bst-scenario-extended.toml

**"IT consultant - extended"** -- exercises all 14 purchase expense codes.

- **Sales**: 52,500 main sales + 2,000 other income across 12 months
- **Purchases**: exercises every code: s (stock), d (fuel), e (employee), p (premises), r (repairs), g (gen admin), m (motor), t (travel), a (advertising), l (legal), b (bank charges), i (interest), o (other), f (fixed assets)
- **Stock**: included (opening/closing values used)
- **Expected P&L**: total sales 52,500, premises 4,800, gen admin 480, legal 850
...
params ──> test ──> generate ──> reconcile (matrix) ──> commit
                                    │
                                    ├── year-end-1: basic + extended
                                    ├── year-end-2: basic + extended
                                    └── year-end-N: basic + extended
```
...
- Runs TWO reconciliations per year-end:
  - `npm run reconciliation -- --package bst --scenario basic --year-end {date}`
  - `npm run reconciliation -- --package bst --scenario extended --year-end {date}`
- **Latest year-end only**: copies the extended-scenario populated xlsx to `examples/bst-latest/GB_Accounts_Basic_Sole_Trader.xlsx`
