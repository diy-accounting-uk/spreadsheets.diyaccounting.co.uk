...
| CQ-38 | The b16 SE agent carried four chained rows in one brief and cost $81.29 for two of them, 341 messages at 299k average context (session GcLg5i). `do-next` says: rows that share files and land in series get one agent each, dispatched fresh from the previous row's commit on the batch, never one agent for the chain | none | machine-only | — | ready-to-start | ~1 file | Haiku | one paragraph in the do-next skill; ~$40 per chained batch |
| CQ-41 | `do-next` requires `npm test -- --all` before a batch's first push (36m39s on b16) when the router's routed run is what the pre-push hook runs and what proved the amended tree in 27s. The rule becomes: the routed run is the first-push proof; `--all` only when the router escalates | none | machine-only | — | ready-to-start | ~1 file | Haiku | shares the do-next skill with CQ-38; 36 min per batch |
| CQ-42 | Twelve docs-only pushes to `main` each ran the router's gates tier (~21s, 4 min in session GcLg5i). `.githooks/pre-push` exits before the router when every path the push adds is `.md`, and says so | none | machine-only | — | ready-to-start | ~1 file | Haiku | hook only; 4 min per session |
| CQ-40 | PR #113's `test` ran 55 min on the head and 57 min again on the merge commit, whose tree differs in `.md` files only (112 of 130 job-minutes). `test.yml` keys a GREEN cache entry by the tree hash of the non-`.md` content (the router's `--tree-hash` over a filtered index) and reuses the PR head's run on the merge commit; a miss runs as today | none | machine-only | — | ready-to-start | ~2 files | Sonnet | CQ-34's marker in CI; 57 job-minutes per merge |
| CQ-39 | The SE agent's first `npm test` was RED after 66 min on eight stale literals: `judge-reconciliation.test.js`, `report-depreciation-parity.test.js`, `se-profit-forecast-checks.test.js`, `diya-gl-shell-money-format.test.js`, `se-warning-checks.test.js` and `se-full-return-checks.test.js` hard-code figures every SE fixture change moves. Derive them from the fixture at test time, as the reconciliation checks do, so a fixture change moves the report and nothing else | none | machine-only | — | ready-to-start | ~6 files | Sonnet | 66 min of agent time per SE fixture change |
