...
Main is `63302827`, green on `d1a68377` (`test`, `deploy`, `codeql`) and prod serves it: PR #111
...
| CQ-29 | `test.yml` and `codeql.yml` run twice per branch push (push and pull_request events) and on every docs-only board commit to main: 17 `test` runs and 19 `codeql` runs for three PRs on 2026-09-13, 1,524 billable job-minutes. Dedupe to one trigger per branch (pull_request for branches, push for main) and add `paths-ignore: ['**.md']` to `test`, `codeql` and `identity-guard`; the router's `TEST_SCOPE_TIERS` already lets a job run gates alone | none | machine-only | — | ready-to-start | ~3 files; workflow YAML; strict-parse before push; Haiku |
