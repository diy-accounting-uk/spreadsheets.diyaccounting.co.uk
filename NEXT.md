...
| H3 | Merge batch PR #67 once WB-1 is in and its checks are green | none | human | WB-1 | blocked-to-start | seven rows code complete; WB-1 joins the same PR |
| WB-1 | The books pages' save menu offers only diya-gl (.zip) and JSON; `download.html` loses its own "Download without donating" link, leaving `donate.html`'s bail-out as the only skip | operator | machine | — | in-flight | Sonnet; worktree save-formats, `claude/ops-save-formats`, lands in batch PR #67 |
