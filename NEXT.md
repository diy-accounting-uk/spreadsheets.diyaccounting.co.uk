...
|---|---|---|---|---|---|---|
| SE-T37a | `addPayrollLine` beside `addBankLine`, the net-and-amount derivation shared with `changePayrollLine`; bundle and MCP edit map | PLAN_DIYA_GL_SE_CLI_MCP_WEB.md | machine | — | in-flight | Sonnet; worktree SE-T37a, `claude/se-payroll-edit`, batch `claude/b2-add-row` |
| SE-T37b | The add row renders the controls the journal's `add` descriptor names; the draft store reads them; a bank row's account select routes by journal | PLAN_DIYA_GL_SE_CLI_MCP_WEB.md | machine | — | in-flight | Sonnet; worktree SE-T37b, `claude/se-add-row`, batch `claude/b2-add-row` |
...
| SE-T37d | The SE manifest's bank, cash and payroll `add` descriptors | PLAN_DIYA_GL_SE_CLI_MCP_WEB.md | machine | — | in-flight | Sonnet; worktree SE-T37d, `claude/se-add-descriptors`, batch `claude/b2-add-row` |
| SE-T37e | The Ltd manifest's bank and payroll descriptors and its payroll chart section | PLAN_DIYA_GL_SE_CLI_MCP_WEB.md | machine | — | in-flight | Sonnet; worktree SE-T37e, `claude/ltd-add-descriptors`, batch `claude/b2-add-row` |
...
| H4 | Merge the b2 batch PR once T37a to T37g are code complete and its checks are green | none | human | SE-T37g | blocked-to-start | draft PR opens with the first merge |
