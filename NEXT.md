...
PR #112 (`claude/b15-board`, worktree `../.worktrees/spreadsheets/b15`, head `165ded7ca`) carries
all eleven rows as one squashed commit each plus the pre-push hook fix; the coordinator's full pass
was green (51m22s). The branch's ci deploy, `test`, `codeql` and `identity-guard` runs are in
progress; `/watch` holds the scope and hands a mergeable PR to `/auto-merge`.
...
|---|---|---|---|---|---|---|---|---|
| SB-3 | The behaviour test asserts which Stripe links each environment serves: ci carries `buy.stripe.com/test_`, prod does not, keyed off `SPREADSHEETS_BASE_URL` | PLAN_DIYA_GL_LAUNCH.md | machine-only | — | in-flight | ~1 file | Haiku | `claude/b15-board`, PR #112, deploy and checks running |
| CQ-31 | `generate-ltd`/`generate-se` run the generate job before the test job, so the tests see the fresh packages; a failed test still blocks the commit job | none | machine-only | — | in-flight | ~2 files | Sonnet | `claude/b15-board`, PR #112, deploy and checks running |
| CQ-29 | `test.yml` and `codeql.yml` fire once per branch push: `push` on `main` only, `pull_request` for branches | none | machine-only | — | in-flight | ~2 files | Haiku | `claude/b15-board`, PR #112, deploy and checks running |
| SET-6 | Design wave for the three SE book records into `PLAN_SE_TEMPLATE_GAPS.md` 3.4: overlap profit (SET-7), single-asset pool marker (SET-8), SBA claim (SET-9) | PLAN_SE_TEMPLATE_GAPS.md | machine-only | — | in-flight | ~1 file | Opus | `claude/b15-board`, PR #112, deploy and checks running |
| CQ-30 | Each b14 agent ran the full router (~45 min, LibreOffice and Playwright) in its own worktree, four at once, driving load to 32–80, timing out unrelated tests and costing ~4h of the batch. The brief's `npm test` line stays, but `do-next` says: an agent verifies with `npm test` on its own diff only (the router already scopes to the diff against `origin/main`; the escalation to the full set on a shallow or detached tree is what bit), and the coordinator runs the one full pass on the merged batch | none | machine-only | — | in-flight | ~2 files | Sonnet | `claude/b15-board`, PR #112, deploy and checks running |
