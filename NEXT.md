...
| TX-T26 | `calculateExpectedTax` rounds `total_tax_and_ni` to the pound while the sheet's calculator and `Income Tax!E17` do not (4,205.50 against 4,206 on the basic book); one rounding, decided against the sheet | PLAN_DIYA_GL_TAXI_CLI_MCP_WEB.md | machine | — | in-flight | `../.worktrees/spreadsheets/tx-t26` on `claude/wt-tx-t26`; Sonnet |
