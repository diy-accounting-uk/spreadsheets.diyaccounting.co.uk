...
| CQ-10 | `deploy.yml`'s Cognito toggle still passes `--client books` in both the enable and disable steps. Submit's script now takes `app|diya-gl|both` and normalises `books` as an alias, so both spellings work today; moving to `--client diya-gl` removes a future cut-over | none | machine | — | ready-to-start | nothing breaks either way; Haiku |
