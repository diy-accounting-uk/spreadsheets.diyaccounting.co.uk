...
| CQ-12 | `app/lib/provenance-data.js` is generated by `scripts/build-provenance-data.mjs` and tracked in git, so every build leaves it modified with a new `engineVersion`. Five sub-agents in one batch each reported it dirty and each declined to stage it, and a sixth will. Generate it and gitignore it, the way `donate.html`, `sitemap.xml` and `catalogue.toml` are, or stop regenerating it in the build steps that do not need it | none | machine | — | ready-to-start | a committed file no one may commit; Sonnet |
| CQ-13 | `web/browser-tests/diya-gl-ltd-warnings.browser.test.js:318` fails under full-suite load and passes 18 of 18 in isolation. The flip assertion above it passes even in the failing run; only the state after `undo(page)` fails to return to baseline, so it is a race in the undo path or in reading the panel after it, not a wrong verdict. It cost two wrong diagnoses in one session, both blaming batch changes that were not responsible | none | machine | — | ready-to-start | a flake that reads as a regression; Sonnet |
