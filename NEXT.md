...
| LP-28 | DIYA-GL's account panel covers the Account button that closes it, so the operator has to scroll to close it. The panel is `position: fixed` at `top: calc(var(--topbar-h) + var(--tabstrip-h) + 0.75rem); right: 1rem` (`web/diya-gl.co.uk/public/diya-gl.css` line 2008), built in `cloud.js` (`openPanel` line 525, `closePanel` line 533, appended to `body` near line 486); on desktop the backdrop that closes it on a click elsewhere is `display: none` (`#account-backdrop`, css line ~2108), so the Account button is the only way out. Diagnose first where the overlap comes from (the topbar or tab strip height not matching the variables, the page scrolled so the topbar moved, or the panel's height), then: a close button inside the panel, Escape closes it with focus back on the Account button, a click outside closes it on every width, and the panel sits clear of the Account button at every width and scroll position. Browser tests in `web/browser-tests/diya-gl-cloud.browser.test.js` for each close path and for the button staying clickable with the panel open | operator 2026-09-25 | machine-only | — | ready-to-start | ~3 files | Sonnet | added 2026-09-25 |
