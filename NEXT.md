...
| LP-1 | The five provenance stamps in `book.toml`'s document info and `report.json`'s header: engine version and commit, tax-data hash, template hash and scorecard, reconciled commit (the format version exists) | PLAN_DIYA_GL_LAUNCH.md | machine | — | in-flight | stamps merged to `claude/b3-launch`; remainder (CI regenerates, generate jobs write reconciledCommit) on `claude/lp-1-reconciled`, worktree lp-1r  |
| LP-2 | The reconciled-releases page: one row per release with the five stamps, linking the per-product reconciliation scorecards | PLAN_DIYA_GL_LAUNCH.md | machine | LP-1 | in-flight | Sonnet, the releases-page agent; `claude/lp-2-releases` off the batch, worktree lp-2  |
...
| LP-5 | The single-file HTML runner per product: engine, schemas, tax data and the product's templates inlined into one stamped page, uploaded beside the zips and linked from `download.html` | PLAN_DIYA_GL_LAUNCH.md | machine | LP-1 | in-flight | Sonnet, the runner agent; `claude/lp-5-runner` off the batch, worktree lp-5  |
...
| LP-16 | In the Submit repo: the storage API in submit-prod: an S3 bucket keyed per user and book, four routes (list, get version, put version, delete) behind the pool's authoriser, a metadata sidecar for optimistic concurrency | PLAN_DIYA_GL_LAUNCH.md | machine | — | in-flight | design landed in `PLAN_DIYA_GL_STORAGE.md` (Submit 92b55766); Sonnet builder on `claude/books-storage-api`, worktree lp-16  |
...
| H11 | Merge PR #69 (`claude/b3-launch`, wave 1 batch) once its checks are green | none | human | G1, LP-1, LP-2, LP-5, LP-6, LP-7, LP-8, LP-9 | blocked-to-start | draft until every wave 1 row is merged into it |
