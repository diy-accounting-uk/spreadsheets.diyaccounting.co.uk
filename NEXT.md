...
| CQ-2 | `deploy.yml` carries `cancel-in-progress: true` on every ref including main, so a second push to main can cancel a prod deploy mid-change | none | machine | H1 | blocked-to-start | not while two PRs are in flight; Sonnet |
