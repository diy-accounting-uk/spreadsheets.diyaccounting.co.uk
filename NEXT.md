...
| SE-T27 | `products/se.js` `checkCompliance` shows four spurious mismatches on a book with no `[expected]` table that nets to a loss | PLAN_DIYA_GL_SE_CLI_MCP_WEB.md | machine | — | in-flight | landed at the four checks; remainder: `calculators/se.js` floors `Income Tax!E6` on a loss |
