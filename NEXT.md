...
| SET-6 | The three SE book records `PLAN_SE_TEMPLATE_GAPS.md` section 7 names, designed then built: an overlap-profit record (brought forward, used, carried) for box 68 and the transition profit 73.3; a single-asset pool marker on `Fixedassets.xlsx!Schedule` rows plus its book field for boxes 50 and 51; an SBA claim record (dates, rate, amount, an array) for boxes 53 and 53.1, with 53.1's row pair laid out between rows 146 and 161 and 73.3 taking row 199's slot once the boxes 69/70 notice moves. Each closes with the derivation filing the box and the warning going. Design wave first, into the plan's 3.4 | PLAN_SE_TEMPLATE_GAPS.md | machine-only | — | ready-to-start | design on Opus, then Sonnet per record; Ltd's shared register moves with the pool marker |
...
- `PLAN_ITSA_SE_DERIVATIONS.md`: its section 8 findings carry `SED-n` ids. SED-7 and SED-8's
  remainder (boxes 68, 73.3, 50/51, 53/53.1) is `SET-6`; SED-10 is Submit's.
