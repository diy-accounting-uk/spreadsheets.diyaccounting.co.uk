...
|---|---|---|---|---|---|---|
| LP-24 | The ci behaviour run switches native sign-in on for the prod DIYA-GL client before the cloud case and off after it, through Submit's `toggle-cognito-native-auth.js enable|disable prod --client books` under the prod role | operator | machine | — | in-flight | on the batch; its cloud case fails on Submit's rejected PUT |
| NM-3 | Rename the `/books/*` public URL namespace to DIYA-GL: `public/books/**`, `download.html`'s hrefs, the manifest and service worker, with `redirects.toml` and CloudFront function entries for every old path | ../submit.diyaccounting.co.uk/PLAN_DIYA_GL_NAMING.md | machine | — | in-flight | `bd2f5027` left the workflow bundle step on the old path |
| NM-4 | Rename the same-repo "books" code identifiers: `books-engine.js`, `books-interchange.js`, `build-books-bundle.mjs`, 46 test filenames, `download.html`'s DOM ids, `books-events.js`, `books.css`, and the `"diya-gl-books"` format string (decided: `diya-gl/1`, the reader keeps accepting the old stamps; needs a version bump and a back-compat reader) | ../submit.diyaccounting.co.uk/PLAN_DIYA_GL_NAMING.md | machine | NM-3 | in-flight | in `naming` after NM-3; the stamp and the back-compat reader |
...
| H1 | Merge the batch pull request for `claude/b7-board` once its checks are green | none | human | LP-24, NM-3, NM-4, NM-2, ITSA-T8 | blocked-to-start | the session may not merge; the branch is testing now |
