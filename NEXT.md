...
which Taxi T5 corrects. R3 and R4 (dispatched 2026-09-05 after Taxi T6 and SE T3) are red on one test,
`ltd-brickwork-pro-nonvat` "keeps the same journal shape on both twins": the CIS-suffered sale T3 added
lands in a different May position in the two Ltd twins; a fix is in flight on
`.worktrees/spreadsheets/fix-twins`, after which both are re-dispatched. Operator, 2026-09-05: no further Ltd rows go in flight until the three running (T20, T4a,
...
|---|---|---|---|---|---|---|
| SE-S4 | S4 `link-caches.js`, the calculator's leaf cells, the stale-cache state | PLAN_DIYA_GL_SE_CLI_MCP_WEB.md | machine | — | in-flight | Fable, `.worktrees/spreadsheets/se-s4` |
| SE-S8 | S8 example books per product from `scripts/example-books.json` | PLAN_DIYA_GL_SE_CLI_MCP_WEB.md | machine | — | in-flight | Haiku, `.worktrees/spreadsheets/se-s8` |
...
| SE-T11 | T11 SE equivalence, formats, round trips (A1–A9, E3–E5) | PLAN_DIYA_GL_SE_CLI_MCP_WEB.md | machine | SE-T7, SE-T8, SE-S4, SE-T14 | blocked-to-start | Opus, wave 8 |
...
| TX-T9 | Taxi T9 extraction map and the sidecar's Taxi baseline | PLAN_DIYA_GL_TAXI_CLI_MCP_WEB.md | machine | — | in-flight | Sonnet, `.worktrees/spreadsheets/taxi-t9` |
| TX-T12 | Taxi T12 headline declaration and the comparison tile | PLAN_DIYA_GL_TAXI_CLI_MCP_WEB.md | machine | — | in-flight | Sonnet, `.worktrees/spreadsheets/taxi-t12` |
...
| LT-T15 | Ltd T15 CLI and MCP harness; Ltd edits in the edit map | PLAN_DIYA_GL_LTD_CLI_MCP_WEB.md | machine | LT-T2, SE-T14 | blocked-to-start | Sonnet, wave 7 |
