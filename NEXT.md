...
PR #127 (DG-1e, DG-1f, OAM-1) merged as `2a6e5b01`; its prod deploy is running. Batch
`claude/b22-board` (worktree `../.worktrees/spreadsheets/b22`) carries DG-2b's agent, with DG-1g
chained after it. PRs #125 and #126 are on prod.
...
| DG-2b | The 24h sandbox on the pages: labels, expiry per book, 403 path removed, ci behaviour case | PLAN_DIYA_GL_HOME.md | machine-only | — | in-flight | ~4 files | Sonnet | agent on claude/dg-2b-sandbox-pages, .worktrees/spreadsheets/dg-2b; batch claude/b22-board |
...
| DG-1g | The homepage: the Ltd example at year view 2025-04, product nav, tier strip, runner row | PLAN_DIYA_GL_HOME.md | machine-only | DG-2b | blocked-to-start | ~11 files | Opus | shares diya-gl.css with DG-2b; chained after it |
