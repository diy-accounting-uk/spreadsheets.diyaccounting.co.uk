...
PR #119 (SET-9) merged as `9fb334c0`; `main`'s test and prod deploy on it in progress. `claude/b16-se`
...
| CQ-44 | `ADMIN_RATE_CELLS` in `app/products/se.js` (line 1738) omits `G6`, so the special-rate WDA renders as money on the DIYA-GL SE page; add `G6`, and `G9`/`G10` once SET-9 lands | none | machine-only | — | ready-to-start | ~1 file | Haiku | SET-9 landed `9fb334c0`; `G6`, `G9`, `G10` |
