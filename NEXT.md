...
| CSP-1 | `img-src` admits only `*.google.com` and `*.google.co.uk`, so GA audiences pixels from other Google ccTLDs (`google.no` seen locally) are blocked | none | machine-only | — | in-flight | ~2 files | Sonnet | agent on claude/csp-1-google-img, .worktrees/spreadsheets/csp-1; batch claude/b21-board |
| DG-3c | Submit: the daily sweeper for lapsed subscribers' resident books (30-day grace) | PLAN_DIYA_GL_HOME.md | machine-only | — | in-flight | ~5 files | Sonnet | agent on claude/dg-3c-lapse-sweeper, .worktrees/submit/dg-3c |
...
| DG-1e | Move the DIYA-GL pages to `web/diya-gl.co.uk/public`; re-home builds, redirects, links and the path constants | PLAN_DIYA_GL_HOME.md | machine-only | — | in-flight | ~40 files | Sonnet | agent on claude/dg-1e-site-move, .worktrees/spreadsheets/dg-1e; batch claude/b21-board |
