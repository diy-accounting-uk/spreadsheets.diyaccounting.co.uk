...
| OAM-1 | `AWS::Oam::Link` in `SpreadsheetsStack` to Submit's us-east-1 metrics sink, gated on the sink ARN | PLAN_SPREADSHEETS_RUM.md | machine-only | — | in-flight | ~4 files | Sonnet | agent on claude/oam-1-metrics-link, .worktrees/spreadsheets/oam-1; batch claude/b21-board |
