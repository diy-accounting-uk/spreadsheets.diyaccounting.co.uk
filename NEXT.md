...
| SE-T28 | The year view carries a mileage-and-CIS strip: Purchases `C2`, `G2`, `A2` (business miles, the month's claim, claim to date) and Sales `W1`, `X1`, Purchases `AD1` (CIS suffered and deducted) per month, 72 cells `render-unrepresentable/se.json` still declares | PLAN_DIYA_GL_SE_CLI_MCP_WEB.md | machine | — | ready-to-start | Sonnet; the month card is the shell's |
