...
|---|---|---|---|---|---|---|
| PL-4 | `verify-roundtrip` aligns the fixture's frame to the export's through `app/lib/period-shift.js`, years included, derived from the two books' declared periods, so the BST, SE and Ltd scorecards pass once the writers shift years (the operator's generate-bst run failed EQ2 with 528 of 528 lines unmatched on date); Taxi's day shift stays | operator | machine | — | in-flight | `../.worktrees/spreadsheets/pl-4` on `claude/wt-pl-4`; Sonnet |
