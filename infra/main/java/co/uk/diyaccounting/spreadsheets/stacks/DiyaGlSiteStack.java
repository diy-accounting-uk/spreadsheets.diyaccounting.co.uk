...
/**
 * DiyaGlSiteStack: S3 + CloudFront for the diya-gl.co.uk static site.
 * <p>
 * Modelled on SpreadsheetsStack with the same pattern:
 * - No Route53 records (those live in root account, managed by root.diyaccounting.co.uk repo)
 * - No Lambda function URL integration
 * - CSP scoped to the DIYA-GL pages: no PayPal, no GitHub avatar/API hosts
 ...
 */
public class DiyaGlSiteStack extends Stack {
    ...
    @Value.Immutable
    public interface DiyaGlSiteStackProps extends StackProps {
        @Override
        Environment getEnv();
        ...
        /** Domain names for the CloudFront distribution (e.g. diya-gl.co.uk, www.diya-gl.co.uk) */
        List<String> domainNames();

        static ImmutableDiyaGlSiteStackProps.Builder builder() {
            return ImmutableDiyaGlSiteStackProps.builder();
        }
    }
    ...
    private static JsonNode loadSecurityHeaders() {
        try (InputStream stream = DiyaGlSiteStack.class.getResourceAsStream("/diya-gl-security-headers.json")) {
            if (stream == null) {
                throw new IllegalStateException("diya-gl-security-headers.json not found on the classpath");
            }
            return new ObjectMapper().readTree(stream);
        } catch (IOException e) {
            throw new IllegalStateException("Failed to read diya-gl-security-headers.json", e);
        }
    }

    public DiyaGlSiteStack(final Construct scope, final String id, final DiyaGlSiteStackProps props) {
        super(scope, id, StackProps.builder().env(props.getEnv()).build());

        String resourcePrefix = props.envName() + "-diya-gl";
        ...
        Tags.of(this).add("Stack", "DiyaGlSiteStack");
        Tags.of(this).add("ManagedBy", "aws-cdk");
        Tags.of(this).add("BillingPurpose", "diya-gl-static-site");
        ...
        // Security headers: values loaded from diya-gl-security-headers.json so the same
        // header set can be replayed by the browser test server in
        // web/browser-tests/serve.js. The CSP allows self and the GA hosts, plus Submit for
        // the cloud sign-in, and nothing else.
        ...
        ResponseHeadersPolicy responseHeadersPolicy = ResponseHeadersPolicy.Builder.create(
                ...
                .comment("Security headers for diya-gl static site")
        ...
        // CloudFront Function for URL redirects (diya-gl.com and www.diya-gl.co.uk → apex)
        ...
        if (Files.exists(redirectFunctionPath)) {
            try {
                ...
                var redirectFunction = Function.Builder.create(this, resourcePrefix + "-RedirectFunction")
                        ...
                        .comment("301 redirects for diya-gl.com and www.diya-gl.co.uk")
                        .build();
        ...
        // Deploy static site files to S3 and invalidate distribution. prune(false) because
        // the offline runners sync into /runners/ separately.
        infof("Using diya-gl doc root: %s".formatted(publicDir));
        ...
        infof("DiyaGlSiteStack %s created for %s", this.getNode().getId(), String.join(", ", props.domainNames()));
    }
}
