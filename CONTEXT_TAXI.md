...
| N16 | `national_insurance.class2_small_profits_threshold` | NI Class 2 small profits threshold (6845); SE Short box 36's caption reads this live |
...
|------|-----------|-----------------|-------------|-----------|
| B5 | Turnover (Total Fares) | `gl-cor:amount (salesTurnover)` | `frs102:TurnoverRevenue` | Box 9 |
| B6 | Fuel | `accounts.purchases.5100` | `dpl:Vehicles` (fuel) | Box 11 (part) |
| B7 | Car Hire / Rental | `accounts.purchases.5200` | `dpl:OperatingLeaseExpenditure` | Box 11 (part) |
| B8 | Repairs & Servicing | `accounts.purchases.5300` | `dpl:OtherRepairsAndMaintenanceCosts` | Box 11 (part) |
| B9 | Road Tax & Insurance | `accounts.purchases.5400` | `dpl:InsuranceCosts` | Box 11 (part) |
| B10 | Capital Allowances | `tax.capitalAllowances` | `ct-comp:TotalCapitalAllowances` | Boxes 23-25 |
| B11 | Mileage Allowance | `tax.mileage (allowance)` | `uk-tax:ApprovedMileageAllowance` | Box 11 (alt) |
| B12 | Total Vehicle Running Costs | `gl-cor:amount (vehicleCosts)` | `dpl:Vehicles` (total) | — |
| B13 | **Gross Profit** | `gl-cor:amount (grossProfit)` | `frs102:GrossProfit` | — |
| B14 | Employee Costs | `accounts.purchases.5500` | `dpl:WagesAndSalaries` | Box 13 |
| B15 | Premises Costs | `accounts.purchases.5600` | `dpl:RentRatesAndServicesCosts` | Box 14 |
| B16 | General Admin | `accounts.purchases.5700` | `dpl:OtherOperationalAndAdministrationCosts` | Box 18 |
| B17 | Advertising | `accounts.purchases.5800` | `dpl:AdvertisingPromotionsAndMarketingCosts` | Box 19 (part) |
| B18 | Legal & Professional | `accounts.purchases.5900` | `dpl:AuditAndAccountancyTaxServices` | Box 16 |
| B19 | Interest & Bank Charges | `accounts.purchases.6000` | `dpl:BankCharges` | Box 17 (part) |
| B20 | Bank Charges | `accounts.purchases.6100` | `dpl:BankCharges` | Box 17 (part) |
| B21 | Other Expenses | `accounts.purchases.6200` | `dpl:OtherCosts` | Box 19 (part) |
| B22 | Total General Expenses | `gl-cor:amount (totalGeneral)` | `frs102:AdministrativeExpenses` | — |
| B23 | **Net Profit** | `gl-cor:amount (netProfit)` | `frs102:ProfitLossOnOrdinaryActivitiesBeforeTax` | — |

Box numbers verified cell by cell against the sheet's own gate formulas (`SE Short!D46 = 'Profit & Loss Acc'!B12-B10`, box 11; `O46 = B18`, box 16; `O51 = B19+B20`, box 17; `O55 = B16`, box 18; `O60 = B17+B21`, box 19; `D55 = B14`, box 13; `D60 = B15`, box 14) and the box numbers the sheet itself prints at `A44`/`L44` through `A68`/`L68`. B12, B13, B22 and B23 are P&L subtotals the return has no box of its own for -- B23 in particular is not box 21: it is the P&L's own bottom line, which agrees with SA103S!D71 (box 21) only when B10 is zero, as the note below the SE Short table explains.
