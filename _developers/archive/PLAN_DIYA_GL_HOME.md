...
| DG-1n | Retarget `/books/` and `/diya-gl/` on the spreadsheets host onto `diya-gl.co.uk`, and the spec page's old path, once the host resolves | 1 | DG-1b | Haiku | `web/spreadsheets.diyaccounting.co.uk/redirects.toml` (~1 file) |
| DG-1j | Cut-over probes on `https://diya-gl.co.uk/` and the old links: a prod behaviour run on the new host plus the redirect and file cases | 1 | DG-1b, DG-1d, DG-1f, DG-1g, DG-1i, DG-1l, DG-1n | Haiku | `behaviour-tests/spreadsheets.behaviour.test.js` (~1 file) |
...
- **DG-1n**: after DG-1b, `web/spreadsheets.diyaccounting.co.uk/redirects.toml`: the `/books/` prefix rule's `to` becomes `/` on target `diya-gl`, and a second `[[redirect_prefix]]` sends `/diya-gl/` to `/` on the same target (DG-1e wrote and then withheld these rules because a 301 to a host that does not resolve takes the pages away from readers; the `diya-gl` target itself is already in the file). `node scripts/build-spreadsheets-redirects.cjs --site spreadsheets` and the redirect unit tests green. Acceptance: DG-1j's case 3.
