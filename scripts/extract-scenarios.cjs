...
// --- Read input files ---
const bookToml = readFileSync(
  path.join(EXAMPLES_DIR, "book.toml"),
  "utf-8",
);
const book = parseTOML(bookToml);

const linesRaw = readFileSync(
  path.join(EXAMPLES_DIR, "lines.jsonl"),
  "utf-8",
);
...
const extendedPath = path.join(
  FIXTURES_DIR,
  "ltd-mar-scenario-extended.toml",
);
...
function countTxns(grouped) {
  ...
  for (const month of MONTH_ORDER) {
    ...
    if (grouped.purchases[month])
      purchasesCount += grouped.purchases[month].length;
  }
...
console.log(
  `  Sales: ${basicCounts.salesCount} transactions, total_sales = ${basicTotalSales}`,
);
console.log(
  `  Purchases: ${basicCounts.purchasesCount} transactions`,
);
console.log("");
console.log(
  `Extended (${extendedPath}):`,
);
console.log(
  `  Sales: ${extendedCounts.salesCount} transactions, total_sales = ${extendedTotalSales}`,
);
console.log(
  `  Purchases: ${extendedCounts.purchasesCount} transactions`,
);
console.log("");
console.log(
  `Full (${fullPath}):`,
...
console.log(
  `  Sales: ${fullCounts.salesCount} transactions, total_sales = ${fullTotalSales}`,
);
console.log(
  `  Purchases: ${fullCounts.purchasesCount} transactions`,
);
console.log(
  `  (Bank transactions from lines.jsonl are included in the full subset as purchases where they map to purchase accounts)`,
);
