...
//
// build-diya-gl-bundle.mjs — one esbuild step that turns the engine into an ES
// module the books page can import, and copies the files that engine reads.
//
// The bundle imports app/lib/diya-gl-engine.js and the pipeline modules behind
...
//
//   node scripts/build-diya-gl-bundle.mjs
...
// The five pages' own script and stylesheet tags, read off the pages
// themselves rather than restated here by hand -- see buildPrecacheManifest().
const PAGES = ["index.html", "bst.html", "se.html", "taxi.html", "ltd.html"];
...
function stubSource(moduleName) {
  ...
  const why = `${moduleName} is not available in the DIYA-GL bundle`;
...
class Ajv2020Stub {
  compile(schema) {
    ...
    throw new Error(
      "Ajv2020.compile(): the DIYA-GL bundle only carries the two published diya-gl schemas precompiled, got $id " + id +
...
export default Ajv2020Stub;`,
  ...
  throw new Error("standaloneCode(): not available in the DIYA-GL bundle. It runs once at build time to generate the precompiled validators the bundle already carries.");
}`,
  };
...
function generateExamplesJs() {
  ...
  writeFileSync(resolve(DIYA_GL_DIR, "examples.js"), examplesJs);
...
function copyRuntimeAssets() {
  ...
  // diya-gl/assets/data/hmrc/form-layouts/se.json and products/taxi-forms.js
...
// generated diya-gl/build-stamp.js that sw.js loads with importScripts() --
...
function toDiyaGlPath(absPath) {
  return relative(DIYA_GL_DIR, absPath).split(sep).join("/");
}
...
function writeBuildStamp() {
  ...
  const source = [
    ...
    "// Generated by scripts/build-diya-gl-bundle.mjs -- do not edit by hand.",
    "//",
    "// sw.js reads this with importScripts() for the cache name to use and the",
    "// list of URLs to precache, so both are derived from what the five pages",
    "// actually load rather than a second, hand-kept copy of the list.",
    `self.DIYA_GL_BUILD_STAMP = ${JSON.stringify(stamp)};`,
    `self.DIYA_GL_PRECACHE_URLS = ${JSON.stringify(urls, null, 2)};`,
    "",
  ].join("\n");
  writeFileSync(resolve(DIYA_GL_DIR, "build-stamp.js"), source);
...
async function main() {
  ...
  mkdirSync(DIYA_GL_DIR, { recursive: true });
  ...
  const result = await build({
    entryPoints: [resolve(ROOT, "app", "lib", "diya-gl-engine.js")],
    outfile: BUNDLE_FILE,
    bundle: true,
    format: "esm",
    platform: "browser",
  ...
  console.log(`DIYA-GL bundle: ${BUNDLE_FILE.replace(ROOT + "/", "")}`);
