...
// never live ones. deploy.yml runs this build step before it builds the
// DIYA-GL engine bundle, so donate-config.js is always in place before
// anything that would precache or serve it.
...
function main() {
  ...
  // diya-gl/shell.js's two donation prompts carry the same £10 link this
  // page's own £10 button does -- one source, so ci can never point a books
  // page at a live payment either.
  const diyaGlConfig = [
    "// SPDX-License-Identifier: LicenseRef-PolyForm-Internal-Use-1.0.0",
    "// Copyright (C) 2006-2026 DIY Accounting Limited",
    "",
    "// Generated by scripts/build-donate-page.mjs -- do not edit by hand.",
    "//",
    "// The Stripe Payment Link diya-gl/shell.js's donation prompts use, from",
    `// donate-links.toml's [${environment}] section.`,
    `window.DIYA_GL_DONATE_LINK = ${JSON.stringify(links.amount10)};`,
    "",
  ].join("\n");
