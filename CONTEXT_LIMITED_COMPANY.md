...
Admin --------------------------------------------------------------------------+
  ...
  | P6/R6/S6/T6/U6 and P7/R7/S7/T7/U7 (CT rates, limits, relief fraction)      |
  | G5-G8 (allowances), G15-G19 (depreciation)                                 |
...
CorporationTax <-- PubP&L!F46 (operating profit)                                |
               ...
               <-- [1]Fixedassets!Schedule (capital allowances)                   |
               <-- Admin rows 6/7 (each financial year's rates) --------------- +
...
| R6, R7 | CT main rate (whole %) | `corporation_tax.main_rate * 100` |
| S6, S7 | Marginal relief fraction | `corporation_tax.marginal_relief_fraction` |
| T6, T7 | Marginal relief lower limit | `corporation_tax.small_profits_limit` |
| U6, U7 | Marginal relief upper limit | `corporation_tax.main_rate_limit` |
...
The full scenario is used in CI matrix reconciliation. The corporation tax charge is checked against how the working sheet builds it: two dated tax rows, one per financial year the accounting period falls in, each taking its share of the chargeable profit and charging it by its own year's figures -- the small profits rate up to its share of that year's lower limit, that year's main rate above it, less marginal relief between the two limits at that year's fraction -- summed into K35. Every figure a row charges by sits on its own Admin row, so a period straddling a change gets each part charged at what that part's year charged. K35 equals the statutory computation at every year end and profit level.
...
| CT: first/second tax row marginal relief = its share of the augmented profits against its share of the limits | CorporationTax L33/L34, F33/F34, A33/A34, A35, K30, Admin S6/S7, T6/T7, U6/U7, P14 | `(U * A/A35/(1+P14) - augmented share) * F / augmented share * S` inside the band, else 0 |
...
| CT: charge for the year = the statutory computation with marginal relief | CorporationTax K35, K28, A33/A34/A35, Admin K6/K7 | `K35` against each financial year's share charged at what that year's own table in `app/data/ltd-<FY>.toml` states (tolerance 1) |
